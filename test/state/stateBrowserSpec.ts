...
describe('state.stateBrowser', function () {
    ...
    describe('resolve', () => {
        var root;
        var browser: StateBrowser;

        function buildChildArray(p: any, prefix: string, count: number, forEach) : any {
            var children = {};
        ...
        beforeEach(mod('dotjem.routing', function () {
            ...
            root.children = buildChildArray(root, 'state', 3, (child) => {
                child.children = buildChildArray(child, 'top', 3, (child1) => {
                    child1.children = buildChildArray(child1, 'mid', 3, (child2) => {
                        child2.children = buildChildArray(child2, 'bot', 3, (child3) => {});
                    });
                });
            });
            browser = new test.StateBrowser(<State>root);
        }));
