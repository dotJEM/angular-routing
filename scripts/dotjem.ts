...
angular.module('dotjem.routing.pages').config([
    <any>'$stateProvider', function (sp:dotjem.routing.IStateProvider) {
        sp.
            state('home', {
                route: '/',
...
angular.module('dotjem.routing.pages').controller('siteController', [
    <any>'$scope', '$state', 'docs', function ($scope, $state, docs) {
        $scope.tab = 'basic';
        $scope.modules = docs;
...
angular.module('dotjem.routing.pages').service('docs', [
    <any>function () {
        var MODULE_DIRECTIVE = /^(.+)\.directive:([^\.]+)$/,
        ...
        angular.forEach(NG_DOCS.pages, (page)=> {
            ...
            switch (page.type) {
                ...
                default:
                    if (match = id.match(MODULE_FILTER)) {
                        ...
                        var service: any = mod.services[key] = mod.services[key] || {};
                        if(match[3]){
